...
async function buscarDaniel() {
  console.log('🔍 Buscando candidato Daniel Lima no Firebase...\n');
  ...
  // Buscar em responses da entrevista 1750029682005 (baseado nos logs)
  console.log('📋 COLEÇÃO: responses (Interview ID: 1750029682005)');
  ...
  responsesSnapshot.forEach((doc) => {
    const data = doc.data();
    if (data.interviewId === 1750029682005) {
  ...
  if (danielResponses.length > 0) {
    console.log(`✅ RESPOSTAS DO DANIEL ENCONTRADAS (${danielResponses.length} respostas):`);
    danielResponses.forEach((response, index) => {
  ...
  // Buscar em candidate-list-memberships para Daniel
  ...
  membershipsSnapshot.forEach((doc) => {
    const data = doc.data();
    // Buscar por ID de candidato que pode ser do Daniel (1750022624973 baseado nos logs)
    if (data.candidateId === 1750022624973 || data.candidateId === 1750022236436) {
...
buscarDaniel().catch(console.error);